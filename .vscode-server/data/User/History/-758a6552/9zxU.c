...
int main(int argc, char **argv, char **env) {
    ...
    while (1) {
        ...
        for (line = strtok(input, "\n"); line; line = strtok(NULL, "\n")) {
            if (check_spaces_tabs(line) && (args = split_string(line, &command))) {
                if (!strcmp(args[0], "exit")) { free_resources(input, args, command); exit(EXIT_SUCCESS); }
                else if (!strcmp(args[0], "env")) print_env();
                else execute_command(argv, args, env);
