...
char **split_string(char *input, char **command) {
	...
	*command = strdup(token);
	if (!(*command)) { perror("Failed to allocate command"); return NULL; }
	args = malloc(buffer * sizeof(char *));
	if (!args) { perror("Failed to allocate args"); free(*command); return NULL; }
	while (token != NULL) {
		if (i >= buffer - 1) {
			buffer *= 2;
			args = realloc(args, buffer * sizeof(char *));
			if (!args) { perror("Failed to reallocate args"); free(*command); return NULL; }
		}
		args[i++] = strdup(token);
		if (!args[i - 1]) {
			while (i--) free(args[i]);
